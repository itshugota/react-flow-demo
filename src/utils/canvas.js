...
export const getCanvasBounding = () => {
  ...
  useReactFlowyStore.getState().nodes.forEach(node => {
    ...
    if (node.width && (node.position.x + node.width) > right) right = node.position.x + node.width;
    if (node.height && (node.position.y + node.height) > bottom) bottom = node.position.y + node.height;

    if (node.shapeType === 'hexagon' && node.shapeData) {
      if (node.shapeData.topPeakHeight && (node.position.y - node.shapeData.topPeakHeight) < top)
        top = node.position.y - node.shapeData.topPeakHeight;

      if (node.shapeData.bottomPeakHeight && (node.position.y + node.height + node.shapeData.bottomPeakHeight) > bottom)
        bottom = node.position.y + node.height + node.shapeData.bottomPeakHeight;
    }
  });
